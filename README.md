...
| 模块 | 说明 |
|---|---|
| 📡 岗位雷达 | 聚合岗位源、多维筛选（企业性质/行业/岗位大类/地点/学历/批次）、一键加入投递计划、手动/每日自动同步 |
| 🗓 今日作战日历 | 按网申截止日期升序的时间轴，可自定义预警阈值（截止前 N 天标橙 / 标红） |
...
| ◫ 知识库 | 文本/文件(Word/PDF/TXT)/链接/图片OCR 录入，按行业/公司/模块归档，关键词搜索，AI 问答自动调取知识库 |
...
2. 岗位演示数据为虚构企业，仅用于功能演示；接入真实数据源时只使用公开可访问信息
3. 简历模板排版为原创设计
